...
export function QuestCard({ title, description, tag, done=false }: { title:string; description:string; tag:string; done?:boolean }) {
  ...
  return <article className="group rounded-3xl border border-white/10 bg-white/[0.035] p-5 transition hover:border-white/20 hover:bg-white/[0.055]">
    ...
    <button onClick={() => setCompleted(true)} disabled={completed} className="mt-6 w-full rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-black disabled:bg-white/10 disabled:text-white/45">
